<template>
   <aside class="absolute inset-0 w-220 z-50 flex flex-col bg-primary-700  text-gray-150">
      ...
      <div class="flex flex-col flex-auto">
         <div v-for="(section, s_i) in Menu" :key="s_i">
            <a v-for="(link, l_i) in Array.isArray(section) ? section : []" :key="l_i"
               class="flex items-center gap-15 p-15 h-45 hover:bg-primary-650" :href="link.Link">
               <i class="text-[15px]" :class="link.Icon" />
               <div v-text="link.Text" />
            </a>
            <div v-if="!Array.isArray(section)" class="flex items-center gap-15 p-15 h-45 hover:bg-primary-650"
               :class="{ 'bg-primary-675': Active === section.Category }" @click="Select(section.Category)">
            ...
            <a v-for="(link, l_i) in (section as any).Links" v-show="Active == (section as any).Category" :key="l_i"
